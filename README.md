...
"#{['Fizz'][100500]}"
```
К слову. Метод `blank?` проверяет, является ли строка пустой. Еще к слову: если вы хотите вызвать какой-либо метод на объекте, генерируемом тернарным оператором, возьмите тернарный оператор в скобки.
...
> Определить функцию от двух аргументов, возвращающую массив целых чисел от одного аргумента до другого, обработанный так:
